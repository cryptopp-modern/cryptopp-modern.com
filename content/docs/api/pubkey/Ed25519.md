...
- Signing without hashing large messages first (Ed25519 buffers entire message)
...
```cpp
static const int SECRETKEYLENGTH = 32;    // Private key (32 bytes)
...
1. **Secure Key Storage:**
   ...
   SecByteBlock privateKey(32);
   signer.GetPrivateKey(privateKey);
   // Auto-zeroed on destruction
   ```

2. **Verify Before Trusting:**
   ```cpp
   if (!verifier.VerifyMessage(...)) {
       // REJECT - do not trust data
       return;
   }
   // Safe to use data
   ```

3. **One Private Key Per Identity:**
   ```cpp
   // WRONG - sharing key across purposes
   ed25519::Signer signer(rng);
   signEmail(signer);
   signSoftware(signer);  // Same key!

   // CORRECT - separate keys
   ed25519::Signer emailKey(rng);
   ed25519::Signer softwareKey(rng);
   ```
...
- [RSA](/docs/api/pubkey/rsa/) - Legacy signatures (coming soon)
