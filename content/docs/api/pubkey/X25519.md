...
int main() {
    ...
    alice.GeneratePublicKey(rng, alicePrivate, alicePublic);

    // Bob generates his key pair
    x25519 bob(rng);
    SecByteBlock bobPublic(x25519::PUBLIC_KEYLENGTH);
    SecByteBlock bobPrivate(x25519::SECRET_KEYLENGTH);
    bob.GeneratePublicKey(rng, bobPrivate, bobPublic);
...
- Skipping the return value check from `Agree()`
- Using with small-order public keys (library validates automatically)
{{< /callout >}}
...
**Important:** Always check return value. `false` indicates small-order attack attempt.
...
kex.Agree(sharedSecret, alicePrivate, bobPublic);
